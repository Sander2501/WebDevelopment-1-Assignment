...
class UserRepository extends BaseRepository implements IUserRepository
{
    ...
    {
        $sql = "INSERT INTO users (email, password_hash, name, phone, created_at) 
                VALUES (:email, :password_hash, :name, :phone, NOW())";

        $stmt = $this->pdo->prepare($sql);
        $stmt->execute(['email' => $email, 'password_hash' => $passwordHash, 'name' => $name, 'phone' => $phone]);
    ...
    {
        $sql = "UPDATE users 
                SET name = :name, phone = :phone, profile_photo = :profile_photo, updated_at = NOW()
                WHERE id = :id";

        $stmt = $this->pdo->prepare($sql);
        $stmt->execute(['id' => $id, 'name' => $name, 'phone' => $phone, 'profile_photo' => $profilePhoto]);
    }
}